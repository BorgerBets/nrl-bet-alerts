...
send_telegram_message("🎯 NRL ROUND - BEST 2-LEG & 3-LEG SGMs ($1.50 - $2.50) 🏉")
...
for game in games:
    ...
    # --- 2. Evaluate and Build ONE Clean 2-Leg SGM (Prioritizing Spread, falling back to Total) ---
    sent_2leg = False

    if best_spread and not sent_2leg:
        ...
        if 1.50 <= odds_2leg_spread <= 2.50:
            ...
            send_telegram_message(msg_2leg)
            sent_2leg = True
            processed_count += 1

    if best_total and not sent_2leg:
...
print(f"Successfully sent {processed_count} streamlined SGM templates.")
